...
<?php 
    ...
    $promotions_pack = $PROMOTION->get_list_promotion_pack_ACTIVE($product_code);
   // echo $wpdb->last_query;

    if($promotions_pack){
        foreach($promotions_pack as $promotion_item){    

            $promotions[] = ['promotion_item' => $promotion_item->promotion_pack_name, 'value' => $promotion_item->voucher_amount]; 
        }
    }
        ...
        <div class="row ">
            ...
            <div class="col-lg-6 col-12">
                   ...
                   <div class="row mb-display-show">
                        ...
                        <div class="mrcenter text-center" >
                            <ul class="product__color mrcenter pd0">
                                <?php foreach ($list_colors as $k => $color) : ?>
                                    <li class="<?php echo $k === 0 ? 'active' : '' ?>" data-index="<?= $k ?>" data-color="<?php echo $color['image_color'];  ?>" data-price="<?php echo str_replace(".","",$color['price_color']); ?>" data-image="<?php echo $color['image_color'];?>" color_code="<?php echo $color['color_code']; ?>" color_name="<?php echo $color['name_color'];?>">
                            ...
                            <ul class="product__color-mb color-list mrcenter pd0">
                                <?php foreach ($list_colors as $k => $color) : ?>
                                    <li class="<?php echo $k === 0 ? 'active' : '' ?>"  data-index="<?= $k ?>" data-color="<?php echo $color['color_show_mobile']; ?>" data-price="<?php echo str_replace(".","",$color['price_color']); ?>" data-img="<?php echo $color['image_color'];  ?>" data-image="<?php echo $color['image_color'];  ?>"  color_code="<?php echo $color['color_code']; ?>" color_name="<?php echo $color['name_color'];?>" style="background: <?php echo $color['color_show_mobile']; ?>"><span></span></li>
                                <?php endforeach; ?>
                            </ul>
                        </div>
                    </div>
                    ...
                    <div class="fz16 colorLGra mb-label-price">Giá từ</div>
                    <div class="product__price-preview p-detail-price" data-price="<?php echo $price ?>" data-price_old="<?php echo $price_old; ?>"  data-sale_off="<?php echo $sale_off; ?>"> 
                        ...
                        <div class="discount-price" <?php if ($price_old == 0) { echo 'style="display: none;"';  } ?> ><?php echo currencyFormat($price); ?></div>
                        <div class="spacer-20"></div>
                    </div> 
                     <?php if ($promotions) : ?> 
                    <div class="spacer-20"></div>
               ...
               <div class="row">
                    ...
                    <div class="col-lg-12 col-12 " >
                        ...
                        <div class="mrcenter text-center" >
                            ...
                            <ul class="product__color-mb color-list mrcenter pd0">
                                <?php foreach ($list_colors as $k => $color) : ?>
                                    <li class="<?php echo $k === 0 ? 'active' : '' ?>" data-index="<?= $k ?>" data-color="<?php echo $color['color_show_mobile']; ?>" data-price="<?php echo str_replace(".","", $color['price_color']); ?>" data-img="<?php echo $color['image_color'];  ?>" data-image="<?php echo $color['image_color'];?>" color_name="<?php echo $color['name_color'];?>"
...
<script type="text/javascript">
    var dealer_id = '', dealer_name='', dealer_address='', color='', size='', color_code='', size_code='', color_name='', size_name='', image, quantity=1, product_code='<?=$product_code?>',price_new=0, price=0, sale_off=0, price_old=0, discount = 0, promotion_code = '';

    color_code = $('.product__color-mb li.active, .product__color li.active').attr('color_code');
    color_name = $('.product__color-mb li.active, .product__color li.active').attr('color_name');
    price = $('.product__color-mb li.active, .product__color li.active').attr('data-price');
    price_old =  $('.p-detail-price').attr('data-price_old');
    sale_off =  $('.p-detail-price').attr('data-sale_off');
    ...
    $(document).ready(function() {
        ...
        $(document).on('click', ".product__color-mb li, .product__color li", function () {
            ...
            // alert(color_code + color + color_name);
...
function get_promotion_product(){
    $.ajax({
        url: ajaxurl, // ajax_get_promotion_product
        data: {
            'action': 'ajax_get_promotion_product',
            data:
            { 
        ...
        success: function(response) {
            ...
            if(datas.length > 0){
                for (let i = 0; i < datas.length; i++) {
                    if(datas[i].discount > 0){
                        discount = parseInt(datas[i].discount);

                    //      promotion_list = '<li class="km_item">'+datas[i].promotion_item_name+' (Giảm '+discount+'% trên giá sản phẩm)</li>';
                    //      $promotions[i] = datas[i].promotion_item_name+' ( Giảm '+discount+'% trên giá sản phẩm)';
                    // }else{
                    //      promotion_list = '<li class="km_item">'+datas[i].promotion_item_name+'</li>';
                    //      $promotions[i] = datas[i].promotion_item_name;

                    }
                   
                }  
                price_old = price;
                sale_off = (price_old*datas[0].discount)/100;
                price = price_new = price_old - sale_off;     

                $(".discount-price").show(); 
                $('.p-detail-price').attr('data-price', price_new);
                $('.p-detail-price').attr('data-price_old', price_old);
                $('.p-detail-price').attr('data-sale_off', sale_off);

                $(".p-detail-price .price").text(currency_format(price_new, '₫'));
                $(".p-detail-price .discount-price").text(currency_format(price_old, '₫')); 
            }else{ 

                $('.p-detail-price').attr('data-price', price);
                $('.p-detail-price').attr('data-price_old', 0);
                $('.p-detail-price').attr('data-sale_off', 0);

                $(".p-detail-price .price").text(currency_format(price, '₫'));
                $(".p-detail-price .discount-price").text('');
                $promotions = Array();
            } 
        }
    });
