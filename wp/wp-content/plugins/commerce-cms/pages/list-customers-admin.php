...
?>
<div class="wrap">
	<h1 class="wp-heading-inline"><?php _e('Customers', 'ccms-customer-list' ); ?></h1>
	<?php
	...
	<form method="get" style="width: 75%;float: right;margin-top: 10px;">
		...
		<p class="search-box">
			<label class="screen-reader-text" for="user-search-input"><?php _e('Search Customers', 'ccms-customer-list' ); ?>:</label>
			<input type="search" id="user-search-input" name="s" value="<?php $searchval = ($search) ? $search : ''; echo $searchval; ?>">
			<input type="submit" id="search-submit" class="button" value="<?php _e('Search Users', 'ccms-customer-list' ); ?>">
		</p>
		<?php if ($customerstatus || $customeraverage || $customerordered || $search): ?>
			<div style="float: right; margin: 5px 15px;"><?php echo $number_of_users; ?> <?php _e('results', 'ccms-customer-list' ); ?></div>
		<?php endif; ?>
	</form>

	<div class="tablenav" style="margin-bottom: 10px;">
		<span style="margin-right: 10px;"><?php echo $displaynum; ?> <?php _e('Customers', 'ccms-customer-list' ); ?></span>
		<form method="get" style="display: inline-block;">
			<input type="hidden" name="page" value="blz-bcl-list-customers" />
			<label for="cus-status" class="screen-reader-text"><?php _e('Filter by Customer Status', 'ccms-customer-list' ); ?></label>
			<select name="cusstatus" id="cus-status">
				<option <?php if (!$customerstatus) { echo 'selected="selected"'; } ?> value=""><?php _e('Active/Inactive', 'ccms-customer-list' ); ?></option>
				<option <?php if ($customerstatus == 'Active') { echo 'selected="selected"'; } ?> value="Active"><?php _e('Active', 'ccms-customer-list' ); ?></option>
				<option <?php if ($customerstatus == 'Inactive') { echo 'selected="selected"'; } ?> value="Inactive"><?php _e('Inactive', 'ccms-customer-list' ); ?></option>
			</select>
			<label for="cus-average" class="screen-reader-text"><?php _e('Filter by if Average Set', 'ccms-customer-list' ); ?></label>
			<select name="cusaverage" id="cus-average">
				<option <?php if (!$customeraverage) { echo 'selected="selected"'; } ?> value=""><?php _e('Average Set/Not Set', 'ccms-customer-list' ); ?></option>
				<option <?php if ($customeraverage == 'set') { echo 'selected="selected"'; } ?> value="set"><?php _e('Average Set', 'ccms-customer-list' ); ?></option>
				<option <?php if ($customeraverage == 'notset') { echo 'selected="selected"'; } ?> value="notset"><?php _e('Average Not Set', 'ccms-customer-list' ); ?></option>
			</select>
			<label for="cus-ordered" class="screen-reader-text"><?php _e('Filter by if have Ordered', 'ccms-customer-list' ); ?></label>
			<select name="cusordered" id="cus-ordered">
				<option <?php if (!$customerordered) { echo 'selected="selected"'; } ?> value=""><?php _e('Ordered/Not Ordered', 'ccms-customer-list' ); ?></option>
				<option <?php if ($customerordered == 'ordered') { echo 'selected="selected"'; } ?> value="ordered"><?php _e('Ordered', 'ccms-customer-list' ); ?></option>
				<option <?php if ($customerordered == 'notordered') { echo 'selected="selected"'; } ?> value="notordered"><?php _e('Not Ordered', 'ccms-customer-list' ); ?></option>
			</select>
			<input type="submit" id="filter-submit" class="button" value="<?php _e('Filter', 'ccms-customer-list' ); ?>">
			<a href="admin.php?page=blz-bcl-list-customers" class="button" style="display: inline-block; margin: 0;"><?php _e('Reset', 'ccms-customer-list' ); ?></a>
		</form>
		<?php
		if ( $page_links ) {
	...
	<table class="wp-list-table widefat fixed striped posts">
			...
			<tr>
				<th scope="col" id="full_name" class="manage-column column-full_name <?php echo ('display_name' == $orderby) ? 'sorted ' : 'sortable '; echo $order; ?>">
					<a href="<?php echo $_SERVER['REQUEST_URI']; ?>&orderby=display_name&order=<?php echo $orderlink; ?>">
						<span><?php _e('Full Name', 'ccms-customer-list' ); ?></span>
						<span class="sorting-indicator"></span>
					</a>
				</th>
				<th><?php _e('Customer Status', 'ccms-customer-list' ); ?></th>
				<th><?php _e('Email', 'ccms-customer-list' ); ?></th>
				<th scope="col" id="avg_order" class="manage-column column-avg_order <?php echo ('customer_average' == $metakey) ? 'sorted ' : 'sortable '; echo $orderclass; ?>">
					<a href="<?php echo $_SERVER['REQUEST_URI']; ?>&orderby=meta_value_num&metakey=customer_average&order=<?php echo $orderlink; ?>">
						<span><?php _e('Avg Order Rate', 'ccms-customer-list' ); ?></span>
						<span class="sorting-indicator"></span>
					</a>
				</th>
				<th><?php _e('Last Order', 'ccms-customer-list' ); ?></th>
				<th scope="col" id="total_orders" class="manage-column column-total_orders <?php echo ('_order_count' == $metakey) ? 'sorted ' : 'sortable '; echo $orderclass; ?>">
					<a href="<?php echo $_SERVER['REQUEST_URI']; ?>&orderby=meta_value_num&metakey=_order_count&order=<?php echo $orderlink; ?>">
						<span><?php _e('Total Orders', 'ccms-customer-list' ); ?></span>
						<span class="sorting-indicator"></span>
					</a>
				</th>
				<th scope="col" id="total_spend" class="manage-column column-total_spend <?php echo ('_money_spent' == $metakey) ? 'sorted ' : 'sortable '; echo $order; ?>">
					<a href="<?php echo $_SERVER['REQUEST_URI']; ?>&orderby=meta_value_num&metakey=_money_spent&order=<?php echo $orderlink; ?>">
						<span><?php _e('Total Spend', 'ccms-customer-list' ); ?></span>
						<span class="sorting-indicator"></span>
					</a>
				</th>
			</tr>
		...
		<tbody>
			...
			if (empty($listusers)) {
				...
				_e('No customers', 'ccms-customer-list' );
				echo '</td></tr>';
			} else {
				...
				foreach ( $listusers as $user ) {
					...
					$orderstotals = 0;
					$cusstatus = __('Inactive', 'ccms-customer-list' );
					$otcal = 0;
					$oscal = 0;
					...
					foreach ($cusorders as $co) {
						...
						if ($i == 1) {
							$cusstatus = $bcl->ccms_get_customer_status($co['postdate']);
					...
					$average = $bcl->ccms_get_order_average($firstorderdate, $lastorderdate, $len);

					if (empty($lastordertext)) {
						$lastordertext = __('No Orders', 'ccms-customer-list' );
						$oscal = 1;
						$otcal = 1;
					}
					...
					if ($oscal == 1) {
						...
						if ( '' === $count ) {
							$orderstotal = '<div class="ccmsuserot" data-user="'.$user->ID.'"><span style="vertical-align: top;">'.__('Calculating', 'ccms-customer-list' ).'</span> <img src="/wp-admin/images/spinner.gif"></div>';
						} else {
					...
					if ($otcal == 1) {
						...
						if ( '' === $spent ) {
							$ordersspend = '<div class="ccmsuseros" data-user="'.$user->ID.'"><span style="vertical-align: top;">'.__('Calculating', 'ccms-customer-list' ).'</span> <img src="/wp-admin/images/spinner.gif"></div>';
						} else {
		...
		<tfoot>
			<tr>
				<td><?php _e('Full Name', 'ccms-customer-list' ); ?></td>
				<td><?php _e('Customer Status', 'ccms-customer-list' ); ?></td>
				<td><?php _e('Email', 'ccms-customer-list' ); ?></td>
				<td><?php _e('Avg Order Rate', 'ccms-customer-list' ); ?></td>
				<td><?php _e('Last Order', 'ccms-customer-list' ); ?></td>
				<td><?php _e('Total Orders', 'ccms-customer-list' ); ?></td>
				<td><?php _e('Total Spend', 'ccms-customer-list' ); ?></td>
			</tr>
		</tfoot>
	</table>
