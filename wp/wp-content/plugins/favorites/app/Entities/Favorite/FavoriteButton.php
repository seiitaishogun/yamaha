...
class FavoriteButton
{
	...
	*/
	public function display($loading = true)
	{
		...
		$html = '<img src="' . get_template_directory_uri() . '/img/ic_heart_o.svg" alt="" />';
		
		if ( !$this->settings_repo->cacheEnabled() ) $loading = false;
		if ( !$this->user->getsButton() ) {
			
			$out = '<a class="simplefavorite-link popup" href="javascript:void(0);" title="'. __('Please login to add favorites.').'" ';
			$out .= ' data-postid="' . $this->post_id . '" data-siteid="' . $this->site_id . '" data-groupid="' . $this->group_id . '" data-favoritecount="' . $count . '" style="' . $this->styleAttributes() . '">' . $html ;
			$out .= '</a>';
			return $out;
			//return false;
		}
