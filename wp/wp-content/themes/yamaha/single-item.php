...
<?php 
	...
	$list_colors = get_field("list_image", $page_id); 
	$list_sizes = get_field("size_code_price", $page_id);
...
<div class="container-fluid" >
  <div class="apparel-detail product_detail_content" data-id="<?php echo $page_id ;?>" data-type="<?php echo get_post_type($page_id); ?>" data-url="<?php the_permalink($page_id) ?>" data-title="<?php echo get_the_title($page_id) ?>"  
  		product_code="<?php if ($list_colors[0]['size_code_price'][0]['sf_product_code'] == '') {echo $product_code;} else {echo $list_colors[0]['size_code_price'][0]['sf_product_code'];}?>"  >
   <?php /*?> <div class="main-detail"><?php */?>
    <div class="row" > 
     		...
     		<div class="main-detail__content col-lg-6 col-12 " >
				...
				<div class="p-detail-price" data-price="<?php if ($list_colors[0]['size_code_price'][0]['price'] == 0) {echo $price;} else {echo ($list_colors[0]['size_code_price'][0]['price']);}?>" data-price_old="<?php echo $price_old; ?>"  data-sale_off="<?php echo $sale_off; ?>">
					<div class="detail-price">
						<span id="p-price" class="price">
							<?php 
								if ($list_colors[0]['size_code_price'][0]['price'] == 0) {
									echo currencyFormat($price);
								} else {
									echo currencyFormat($list_colors[0]['size_code_price'][0]['price']);
								}
							?>
				...
				<div class="p-detail-color product__color">
					...
					<div class="list-color color-list">
						...
						?>
								<a class="<?php echo $k === 0 ? 'active' : '' ?> load-color" href="javascript:void(0)" data-index="<?php echo $k ?>" data-color="<?php echo $item['color']; ?>" color_code="<?php echo $item['color_code']; ?>" color_name="<?php echo $item['color_name']; ?>" data-price="<?php echo $price; ?>" data-image="<?php echo $item['image'][0]['sizes']['medium']; ?>">
				...
				<div class="p-detail-size">
					...
					<ul class="list-size size-list">
						<?php
						if ($list_colors[0]['size_code_price']) :
							foreach ($list_colors[0]['size_code_price'] as $k => $size) :
						?>
								<li class="<?php echo $k == 0 ? 'active' : '' ?>" >
									<a  size_code="<?php echo $size['size_code']; ?>" 
										size_name="<?php echo $size['size_code']; ?>" 
										data-size="<?php echo $size['size_code']; ?>"
										data-size="<?php echo $size['size_code']; ?>"
										product_code="<?php echo $size['sf_product_code']; ?>"
										price="<?php echo $size['price']; ?>"
										class="" 
										href="javascript:void(0)"><?php echo $size['size_code']; ?> 	
									</a>
								</li>
							<?php endforeach; ?>
						<?php endif; ?>
            ...
            <div class="swiper-container">
                <div class="swiper-wrapper">
                    ...
                    ?>
                            ...
                            $price_other = get_field('price', $post->ID);
                            ?>
                            <a class="swiper-slide product-item <?php echo $sale;
                                ...
                                <div class="product-item__price"><?php echo currencyFormat($price_other); ?></div>
                            </a>
...
<script type="text/javascript">
	var product_code='<?=$product_code?>', color_code='', size='', size_code='', color='', color_name='',  size_name='', price=<?=$price?>, sale_off=<?=$sale_off?>, price_old=<?=$price_old?>, dealer_id = '', dealer_name='', dealer_address='', quantity=1, image = '', promotion_code = '' ;
    ...
    price = $(".p-detail-price").attr("data-price");
    product_code = $('.product_detail_content').attr('product_code');
...
//            })
        ...
        $(document).on("click", ".color-list a", function() {
         ...
         $(document).on("click",".list-size li", function(e) {
            ...
            if ($(".list-size li.active a").attr('product_code') != '') {
            	price = $(".list-size li.active a").attr('price');
            }
	...
	get_promotion_product(product_code);
	function get_promotion_product(pro_code){
	        ...
	        success: function(response) {
	            ...
	            if(datas.length > 0){
	            	for (let i = 0; i < datas.length; i++) {
	            		promotion_list = '<li class="mb5">'+ datas[0].promotion_item_name +'</li>' + promotion_list;
	            		if (datas[i].discount != '' || datas[i].discount != undefined) {
	            			discount = parseInt(datas[i].discount);
	            		}
	            	}
	            	if (discount > 0 ) {

	            		price = $(".list-size .active a").attr("price");
	            		...
	            		promotion_code = datas[0].promotion_item_id;
            		}
	            }else{
	            	discount = 0;
	            	price = '<?=$price?>';
