...
global $wpdb, $current_user, $current_user_profile, $ListBookings, $BookingItems, $cart_count, $current_customer, $current_customer_address;
...
function send_email_order_to_customer($user_email, $customer_list_orders = false){
	global $current_user, $current_user_profile, $C_ORDER;
	if($customer_list_orders && $current_user){  
		...
		// set data for email template
		$receiver_address = json_decode($customer_list_orders[0]['rec_address']);
        $email_data['fullname'] = $current_user_profile->full_name;
        $email_data['email'] = $current_user_profile->email;
        $email_data['phone'] = $current_user_profile->phone;
        ...
        // get order detail
        // product_name, product_price, product_price_original, quantity, discount
        $email_data['order_items'] = [];

        $email_data['order_url'] = get_site_url() . '/user/?order_id=' . $customer_list_orders[0]['ID'] . '&ui=' . $current_user->ID . '&tk=' . $token . '#check-order';
        for ($i=1; $i <count($customer_list_orders) ; $i++) { 
        	$detail = $C_ORDER->get_order_details_by_ORDER_ID($order->ID);
        }
			...
			if ( $userID ) {
				...
				if($res && $res['data']){
					...
					if($res['data']['province']){
						$province = $wpdb->get_row( "SELECT * FROM {$wpdb->prefix}province WHERE sf_code = {$res['data']['province']} LIMIT 1 ; ");
						if($province)$data['province_id'] = $province->id;
					}	
					if($res['data']['district']){
						$district = $wpdb->get_row( "SELECT * FROM {$wpdb->prefix}district WHERE sf_code = {$res['data']['district']} LIMIT 1 ; ");
				...
				if(intval($data_customer['ID']) > 0){
					$cus_ID = $wpdb->update($wpdb->prefix.'customer', $data_customer, array( 'ID' => $data_customer['ID'] ));
				}
				else{			
...
function update_sub_order_and_Items($data_order, $data_Items){
	...
	if(intval($data_order['parent_id']) > 0 ){ 
		...
		if(count($data_Items['dealerBike']) > 0){
			...
			foreach($data_Items['dealerBike'] as $dealer_item){
				$data_item = $dealer_item;
		...
		if(count($data_Items['dealerPCA']) > 0 ){
			...
			foreach($data_Items['dealerPCA'] as $dealer_items){
				...
				if($insertID > 0 && is_array($dealer_items)){
					$order_ids[] = $insertID;
					$totals = 0;
					$paid = 0;
					$discount = 0;
...
function update_order_booking_splited(){
	...
	$data_order = array(
			  ...
			  'voucher' 			=> sanitize_text_field($data_customer['voucher']), 
...
// Insert/Update order_booking_details
function update_order_booking_details(){
	global $wpdb, $current_user_profile, $current_user;
	...
	if($order_id <= 0 && !$orders_id){
		wp_send_json_success(array('success'=> 0,'order_id' => $order_id));
		die();
	}
	
	$data_orderItemarr = $_POST['dataorderItem'];
	$data_orderItemarr1 = $data_orderItemarr;	 
	...
	$sub_total = 0;
	$totals = 0;
	for($i=0;$i<count($data_orderItemarr1);$i++){
		
		$data_orderItem = $data_orderItemarr1[$i] ;
		
		$data_orderItem['price'] = str_replace(".", "", $data_orderItem['price']);
		$data_orderItem['price_old'] = str_replace(".", "", $data_orderItem['price_old']);
		$data_orderItem['sale_off'] = str_replace(".", "", $data_orderItem['sale_off']);
		$data_orderItem['quantity'] = str_replace(".", "", $data_orderItem['quantity']);
		$data_orderItem['deposit'] = str_replace(".", "", $data_orderItem['deposit']);
		$itemid = $data_orderItem['item_id'];
		
		$sub_total = (intval($data_orderItem['price']) * intval($data_orderItem['quantity'])) - intval($data_orderItem['sale_off']);
		 
		if(intval($data_orderItem['deposit']>0)){
			$sub_total = (intval($data_orderItem['deposit']) * intval($data_orderItem['quantity'])) - intval($data_orderItem['sale_off']);
		} 
		$data_orderItem['sub_total'] = $sub_total;
		
		$totals += $sub_total;
		
		$str_des = sanitize_text_field($data_orderItem['dealer_name'].'@'.$data_orderItem['dealer_address']) ;
		
		$current_items = $wpdb->get_results( "SELECT * FROM $order_details 
		WHERE order_id = '$order_id' and item_id = '$itemid' LIMIT " . count($data_orderItemarr1) ." ;");
		
		$update  = false; 
		$id = 0 ;
		if ( ! empty( $current_items ) ) {
			if(isset( $current_items[0]->ID))
		...
		$postarr = array(
				'ID' 		=> $id, //intval($data_orderItem['ID']),
			  	'order_id' => intval($order_id), 
				'dealer_id' => intval($data_orderItem['dealer_id']),
				'dealer_name' => sanitize_text_field($data_orderItem['dealer_name']),
				'dealer_address' => sanitize_text_field($data_orderItem['dealer_address']),
				'item_id' 		=> sanitize_text_field($data_orderItem['item_id']), //Item của Product
				'sku' 		=> sanitize_text_field($data_orderItem['item_id']), //Item của Product
			  	'quantity' 		=>	intval($data_orderItem['quantity']),			  
			  	'deposit' 	=>	intval($data_orderItem['deposit']),			  
			  	'price' 	=>	intval($data_orderItem['price']),
			  	'price_old' 	=>	intval($data_orderItem['price_old']),
			  	'sale_off' 	=>	intval($data_orderItem['sale_off']),
			  	'sub_total' =>	$data_orderItem['sub_total'],
				'description'=>	sanitize_text_field($order_id . '@'. $str_des) ,
			  	'sf_order_id' => $order_id,
				'product_type' => 	sanitize_text_field($data_orderItem['product_type']),
			  	'product_code' => 	sanitize_text_field($data_orderItem['product_code']),
			  	'sf_status' => 1,
			  	'size' 		=> sanitize_text_field($data_orderItem['size']),
			  	'color' 	=> sanitize_text_field($data_orderItem['color']),
			  	'image' 	=> sanitize_text_field($data_orderItem['image']),
				'post_ID' 	=> $data_orderItem['item_id'], //Item của Product
				'date_updated' => current_time( $type='mysql', $gmt = true), 
			);
	...
	// Are we updating or creating?
		...
		$insertID = $id;
		if($update  == false){ //insert data 
			$postarr['date_created'] = current_time( $type='mysql', $gmt = true);
			$success += $wpdb->insert($order_details, $postarr); 
			$insertID = $wpdb->insert_id;
		}else{ //update data 
			$postarr['date_updated'] = current_time( $type='mysql', $gmt = true);
			$success += $wpdb->update($order_details, $postarr, $where=array('ID' => $postarr['ID']));
		} 
		
	}  
	
	if ($order_id >0 && $success>0) { // XOA COOKIE, update order status = 1, order_total
		$wpdb->update($table_orders, array('ID' => $order_id, 'order_status' => 1, 'order_total'=>$totals), $where=array('ID' => $order_id));
		//unset($_COOKIE['booking_cookie']); 
	}
	wp_send_json_success(array('order_id'=>$order_id, 'item_id'=>$insertID, 'update'=>$update, $wpdb->last_query, $data_orderItemarr1 ) );
	die();
}

add_action('wp_ajax_update_order_booking_details', 'update_order_booking_details'); // wp_ajax_{action}
add_action('wp_ajax_nopriv_update_order_booking_details', 'update_order_booking_details'); // wp_ajax_nopriv_{action}
...
$(document).ready(function(){
		...
		var item_id = '<?php echo get_queried_object_id(); ?>', 
			...
			if (product_type == 'item'){
				size = (size)?size: $('.list-size .active a').html(); 
				size_code = (size_code)?size_code: $('.list-size .active').attr('size_code'); 

				color = (color)?color:$(".product__color .active").attr('data-color');
				color_code = (color_code)?color_code:$(".product__color .active").attr('color_code');
			}
		...
		var subtotal = (quantity * price) - (sale_off) ;   
	...
	function checkDealerAddToCart(){
		...
		if(dealer_id==''){ 
			...
			if( DATA_CARD != null && DATA_CARD.length > 0 && item_index < 0){
				showPopup('<div class="colorDark fnt-18 text-center pt20 pb40"><h4 class="colorDark fnt-oswald fnt-22 pb20">Thông báo:</h4><p>Thời gian xử lý đơn hàng có thể sẽ chậm hơn so với dự kiến. Do có hai sản phẩm mà bạn chọn được thuộc hai Đại lý khác nhau.</p></div> <button type="button" id="btnBack" class="btn-clip btn-red btn-small">Quay lại</button> <button type="button" id="btnNext" class="btn-clip btn-border-red btn-small">Tiếp tục</button>', el=$('.popup_content'));
...
function addToCard(data){
	...
	if(old_data){
		...
		var item_index = old_data.findIndex(x => (x.item_id === data.item_id && x.dealer_id === data.dealer_id && x.color === data.color && x.size === data.size));
		// XE
...
<script type="text/javascript">
    ...
    $(document).ready(function() { 
		...
		$("button.btn_plus_minus").click(function(e) {
			...
			//alert('quantity: '+product_type);
			
			if(deposit=='' || deposit == undefined){ deposit=0;}
			
			if(product_type=='bike' || product_type =='product' || deposit == 0){
				deposit=10000000;
			}
...
function update_Promotion_ToCart(data_cart, data_promotion){
	...
	$.each(data_promotion, function(index, promotion_item){
		//alert(promotion_item.record_type.toLowerCase());

		if(promotion_item.record_type.toLowerCase() == 'bike'){
		...
		}else if(promotion_item.record_type.toLowerCase() == 'pca'){
			var item_index = data_cart['dealerPCA'].findIndex(x =>(x.product_code == promotion_item.product_code));

			if(item_index >=0){

				var cart_item = data_cart['dealerPCA'][item_index];
				...
				if(amount > 0){
					...
					cart_item.price -= amount;
					cart_item.subtotal -= amount * cart_item.quantity;
					data_cart['dealerPCA'][item_index] = cart_item;
				} 
			} 
		}		 
	...
	console.log('Line: 2172: End update_Promotion_ToCart...');
	console.log(data_cart);
 ...
 function removeToCard(data){
	...
	if(cart_count>0){
		 $(".mcart_count").find('.c_count').html(cart_count).show();
	 }else {$(".mcart_count").find('.c_count').html(cart_count).hide();}
...
function render_CartItem(Data_Item){
	...
	// Hinh Anh
	strItem += '<div class="col-md-2 col-4 align-self-center align-items-center item-cart-img">';
	strItem += '<a href="@permalink" class="product-link" >';
	strItem += '<img src="@image; ?>" class="box-img lazyload" /></a>';
	strItem += '</div>';
	
	
	strItem += '<div class="col-md-7 col-8 align-self-center align-items-center item-cart-detail-box">';
	...
	// Title - Dealer
	strItem += '<div class="col-md-5 col-12 align-self-center align-items-center item-cart-detail-info" > ';
	...
	if(parseInt(Data_Item.sale_off) > 0){
		strItem += '<p class="sale_off">size: '+currency_format(Data_Item.sale_off, 'đ')+' </p>';
	} 
...
function render_booking_checkout(Booking_Table = $('#Booking_Table'), Booking_Items = $('#Booking_Items'), data_cart_in ){
	var section_total = '';
	var section_voucher = '';
	...
	if(DATA_CARD.length > 1){ 
		...
		section_voucher += '				<input type="text" id="voucher_input" class="bg_eee border0 text-center fnt-oswald color333 fnt-18 fw600 height40" placeholder="Nhập mã giảm giá" value="" />';
		section_voucher += '					<div class="clearfix"><button id="btn_check_voucher" class="btn btn-clip btn-red btn-small hide fnt-12 pd0" >Kiểm tra</button></div>';
		section_voucher += '					<span class="txtVoucher colorRed small text-right">';
		 
		section_voucher += '					</span>';
		section_voucher += '				</div>';
		section_voucher += '				</div><div class="clearfix"></div>';
		...
		//GIAM GIA
		if($saleoff > 0 ){
			section_total += '		<div class="col-md-12 item-cart-total-box sale_off">';
...
function hidden_ecommerce_website(){
	...
	var birthday = new Date('2022-03-26 19:45:59'); 
	...
	if(timebirthday >= timetoday){ 
	 	window.location.href='<?php echo get_site_url()?>/homepage/';		 
	}
