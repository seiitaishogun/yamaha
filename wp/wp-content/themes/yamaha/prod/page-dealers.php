...
<div class="banner-single-full bg" style="background-image: url('<?php echo get_template_directory_uri() ?>/img/dealers/img-banner-dealers.jpg')">
    <div class="container-fluid">
        <div class="content-banner">
            <h1 class="title ff-1 text-uppercase">Tìm đại lý gần bạn</h1>
        </div>
    </div>
</div>
...
<div class="dealers-address">
    <div class="container-fluid">
        <div class="row">
            <div class="col-md-5">
                <h2 class="title ff-1 text-uppercase">Đại lý</h2>
                <div class="group-dropdown">
                    <div class="dropdown dropdown--selected filter-provine">
                        <a href="javascript:void(0)" class="btn" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">
                            ...
                            <div class="title">Chọn Tỉnh Thành</div>
                        </a>
                        <div class="dropdown-menu" aria-labelledby="dropdownMenuButton" style="">
                            <?php foreach ($location as $e) : ?>
                                <?php if ($e->parent == 0) : ?>
                                    <a class="dropdown-item" href="javascript:void(0)" data-provine="<?php echo $e->term_id; ?>"><?php echo $e->name ?></a>
                                <?php endif ?>
                            <?php endforeach; ?>
                        </div>
                    </div>
                    <div style="width: 8px"></div>
                    <div class="dropdown dropdown--selected filter-town">
                        <a href="javascript:void(0)" class="btn" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">
                            <div class="label">Quận Huyện</div>
                            <div class="title">Chọn Quận Huyện</div>
                        </a>
                        <div class="dropdown-menu" aria-labelledby="dropdownMenuButton"></div>
                    </div>
                </div>
                <p class="text-result"><span id="result">0</span> kết quả</p>
...
<script type="text/javascript">
    ...
    $(document).ready(function() {
        ...
        $(document).on('click', '.filter-provine .dropdown-item', function() {
            var that = $(this);
            var text = that.text();

            filter_provine = that.attr('data-provine');

            $(".filter-provine .title").text(text);

            if(filter_provine == that.attr('data-provine')) {
                $(".filter-town .title").text("Chọn Quận Huyện");
            }

            $.ajax({
                url: ajaxurl, // AJAX handler
                data: {
                    'action': 'filter_dealer',
                    'provine': filter_provine,
                },
                type: 'POST',
                beforeSend: function(xhr) {
                    $(".list-address .item-address").remove();
                    $(".filter-town .dropdown-menu a").remove();
                },
                success: function(data) {
                    if (data) {
                        var parseData = $.parseJSON(data);

                        $(".filter-town .dropdown-menu").append(parseData.nav);
                        $(".list-address").append(parseData.data);
        ...
        $(document).on('click', '.filter-town .dropdown-item', function() {
            var that = $(this);
            var text = that.text();
            var filter_town = that.attr('data-town');

            $(".filter-town .title").text(text);

            $.ajax({
                url: ajaxurl, // AJAX handler
                data: {
                    'action': 'filter_dealer_town',
                    'provine': filter_provine,
                    'town': filter_town,
                },
                type: 'POST',
