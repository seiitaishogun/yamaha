...
function loadmoreApparel_ajax_handler()
{
    ...
    foreach ($posts as $item) {
        ...
        <div class="product-item__price">' . currencyFormat($price) . '</div>
    </a>';
                            ...
                            <div class="block-service">
                                ...
                                <div class="d-md-flex justify-content-between w-100">
                                    <div class="info">
                                        ...
                                        <p class="price">' . currencyFormat(get_field('price', $e->ID)) . '</p>
                                    </div>
...
function filter_items_ajax_handler()
{
    ...
    foreach ($posts as $item) {
        ...
        $price = intval(get_field('price', $item->ID));
        $price_old = intval(get_field('price_old', $item->ID));
        $list_sizes = get_field("list_image", $item->ID);

        foreach ($list_sizes[0]['size_code_price'] as $k => $size){
            $rows_promotion = $PROMOTION->get_promotion_product_by_FIELD($FIELD = 'product_code', $value = $size['sf_product_code']);

            if(intval($rows_promotion->discount) > 0 ){                
                $price_old = $price;
                $price = $price - (intval($rows_promotion->discount)*$price)/100;
                $discount = intval($rows_promotion->discount);
            }
        ...
        if($discount > 0){
            $out .= '<div class="product-item__price">' . currencyFormat($price) . ' <span class="price_old">' . currencyFormat($price_old, '₫') . '</span></div>';
                $out .= '<span class="product-item__percent">Giảm ' . $discount . '%</span>';
            
        }else{
            $out .= '<div class="product-item__price">' . currencyFormat($price) . ' </div>';
        }
...
function filter_package_ajax_handler()
{
    ...
    foreach ($posts as $k => $item) {
        ...
        $nav .= '<li class="' . $active . '"><a href="#">' . get_field('name_bike', $item->ID) . ' (' . currencyFormat($price) . ')</a></li>';
        ...
        <p class="price">' . currencyFormat($price) . '</p>
        <div class="image">
        ...
        <div class="background-image" data-toggle="modal" data-target="#packageModal' . $item->ID . '" style="background-image: url(' . get_field('image', $item->ID) . ');">
            <div class="d-md-flex">
                ...
                <div class="info">
                    ...
                    <div class="colorLine fz14">' . currencyFormat($price) . '</div>
                </div>
            </div>
        </div>
    </div>';
...
function find_ajax_handler()
{
    ...
    if ($bikes) {
        foreach ($bikes as $post) {
            ...
            $out_bike .= '<div class="product-item item-25"><a href="' . get_permalink($post->ID) . '" class="stretched-link"></a><div class="product__image"><img src="' . get_field("feature_product", $post->ID)['feature_img'] . '" alt=""></div><div class="product__content"><div class="title">' . get_field("feature_product", $post->ID)['feature_title'] . ' ' . get_field("feature_product", $post->ID)['feature_type'] . '</div><div class="text colorLGray">From <span class="fz20">' . currencyFormat($price) . '</span></div></div></div>';
        }
    }
    if ($apparel) {
        $sale = '';
        ...
        foreach ($apparel as $k => $item) {
            ...
            $out_apparel .= '<a class="product-item ' . $sale . ' ' . $hide . '" href="' . get_permalink($item->ID) . '">
                <img src="' . get_field("list_image", $item->ID)[0]['image'][0]['sizes']['medium'] . '" alt="" />
                <div class="product-item__title">' . get_the_title($item->ID) . '</div>
                <div class="product-item__price">' . currencyFormat($price_ap) . '</div>
            </a>';
...
function gt_posts_custom_column_views($column)
{
    if ($column === 'post_views') {
        echo get_post_meta(get_the_id(), 'post_views_count', true ) . ' lượt xem';
    }
}
