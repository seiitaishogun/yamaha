            ...
            <?php  if($step == 2){
				...
				if(isset($_POST['emailReset'])): 
					...
					if($u_id && $us->user_activation_key == $token){
						...
						$email_data['url'] = get_site_url().'/reset-password/?step=3&ui='.$u_id.'&tk='.$token;
						$message_body = render_email($email_data, CCMS_ABSPATH . '/html/email/forgot_password.php');


						$send_success = wp_mail( $emailto, $subject, $message_body, $headers, $attachments='' );

						if($send_success){
							echo '<p class="text-danger px-5">Hệ thống vừa gửi thông tin xác nhận cấp lại mật khẩu cho bạn. Vui lòng kiểm tra email và nhấn nút <b class="colorBlue" >xác nhận</b>.</p>';
						}else{
